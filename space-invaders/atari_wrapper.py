...
def wrap_deepmind(
    ...
    :param bool render: render the environment.
    :return: the wrapped atari environment.
    """
    assert 'NoFrameskip' in env_id
    env = gym.make(env_id, render_mode='human') if watch else gym.make(env_id)
...
def make_atari_env(task, seed, training_num, test_num, watch, **kwargs):
    ...
    if envpool is not None:
        ...
        env = wrap_deepmind(task, watch=watch, **kwargs)
        train_envs = ShmemVectorEnv(
            [
                lambda:
                wrap_deepmind(task, episode_life=True,
                              clip_rewards=True, watch=watch, **kwargs)
        ...
        test_envs = ShmemVectorEnv(
            [
                ...
                wrap_deepmind(task, episode_life=False,
                              clip_rewards=False, watch=watch, **kwargs)
